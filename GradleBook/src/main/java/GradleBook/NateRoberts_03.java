...
/**
   Project 3: Main class for Gradebook aplication
   @author Nate Roberts
   @version 1.0
   ...
   stochastic (adj.) - Pertaining to conjecture; Following some random 
      probability distribution or pattern, so that its behavior may be
      analysed statistically, but not predicted precisely.
   
   "If it's your job to eat a frog, it's best to do it first thing in the
      morning.  And if it's your job to eat two frogs, it's best to eat
...
public class NateRoberts_03 {

   // Static variables
   private static List<Student> listOfStudents;       // List object to store
                                                      // all Student objects
   private static List<GradeItem> listOfGradeItems;   // List object to store
   ...
   public static void main(String[] args) throws IOException {
      ...
      final String INPUT_FILE = "hw3input" + xx + ".txt";
                     // Construct input file name using command-line arguments
      final String OUTPUT_FILE = "hw3output" + xx + ".txt";
                     // Construct output file name using command-line arguments
      listOfStudents = new List<Student>();     // Initialize list of Students
      listOfGradeItems = new List<GradeItem>(); // Initialize list of GradeItems
   ...
   */
   public static void generateReport(String outFile) throws IOException {
      ...
      for (int s = 0; s < listOfStudents.getCurrentSize(); s++) {
         currentStudent = listOfStudents.get(s);
         maxTotal = 0;
         actualTotal = 0;
         overall = 0.0;
         ...
         out.println("   Grade Items");
            ...
            for (int i = 0; i < listOfGradeItems.getCurrentSize(); i++) {
               currentItem = listOfGradeItems.get(i);
